...
// Views
// Utils
var actions_createValidator=function createValidator(_ref){var address=_ref.address,deposit=_ref.deposit,hash=_ref.hash,node=_ref.node;return function(dispatch,getState,_ref2){var account=_ref2.account,web3=_ref2.web3;dispatch({type:CREATE_VALIDATOR_REQUEST});return new Promise(function(resolve,reject){var value=web3.utils.toWei(deposit,'ether');account.methods.addInitialDeposit("0x".concat(hash.replace(/^(0[xX]{1})?/,'')),"0x".concat(node.replace(/^(0[xX]{1})?/,'')),"0x".concat(address.replace(/^(0[xX]{1})?/,''))).send({value:value,from:window.ethereum.selectedAddress}).on('receipt',function(){return dispatch(actions_updateValidator(hash,{isFetching:false}));}).on('error',reject).on('transactionHash',function(){resolve();dispatch({type:CREATE_VALIDATOR_SUCCESS,hash:hash,payload:{address:address,hash:hash,node:node,deposit:convertDeposit(deposit),isFetching:true,pauseCause:1}});dispatch(actions_closeModal(VALIDATOR_CREATE_MODAL_ID));document.getElementById('0xac85685277fba446c42272538fa64f3b28bd42a5a8b6f2295b7f2a22ee10bf35').scrollIntoView();});}).catch(function(error){// eslint-disable-next-line
...
var components_Metamask = __webpack_require__(64);
...
//# sourceMappingURL=main.296a626f.chunk.js.map